...
8.  Run the command in **E:\Labfiles\Lab03\SqlPackageCmd.txt**, replacing **\<your db servername\>** with the server name you created earlier (\<*yourname\>\<date*\>srv).
9.  The database can take 5-10 minutes to import, when the import completes successfully, close the command prompt.
10. Return to the Azure Portal in Internet Explorer.
11. In the Azure Portal, in the Navigation pane, click **All resources**, and then click **WideWorldImporters-Standard**.
12. Under **Overview**, click **Query editor (preview)**.
13. On the **Query editor (preview)** blade, enter the following details, and then click **OK**:
    - **Authorization type**: SQL server authentication
    - **Login**: dbadmin
    - **Password**: Pa55w.rd

14. Click in line 1, type the following commands, and then click **Run**:
...
15. Delete the commands.
16. Click in line 1, type the following command, and then click **Run**:
    ...
    >**Note**: The above command can be copied from: E:\Labfiles\Lab03\SqlViewCmd.txt. The monetary columns are converted to floating point values because the AFx library used by Azure Machine Learning services does not support the SQL Server DECIMAL type.

17. In Internet Explorer, in the address bar, type **https://studio.azureml.net/**.
18. On the **Microsoft Azure Machine Learning Studio** page, click **Sign In**. 
19. If you are prompted for credentials, use the details of the Microsoft account that you created for this course.
20. Ensure that **EXPERIMENTS** is selected in the navigation pane, then click **+ NEW**.
21. Click **Blank Experiment**.
22. In the **Search experiment items** box, type **Import**, and then from the module list, drag **Import Data** on to the experiment canvas.
23. In the right-hand **Properties** pane, in the **Data source** list, select **Azure SQL Database**.
24. In the **Database server name** box, type **\<*yourname*\><\*date*\>srv.database.windows.net**, replacing **\<*yourname\>\<date*\>srv** with the name of your database server
25. In the **Database name** box, type **WideWorldImporters-Standard**.
26. In the **User name** box, type **dbadmin**.
27. In the **Password** box, type **Pa55w.rd**.
28. In the **Database query** box, type the following command to use a subset of the total data:

    ````SQL
    SELECT * FROM [20774A].[CustomerTransactions]
    ````
    
29. Select **Use cached results**.
30. Click **SAVE AS** at the bottom of the page.
31. In the **EXPERIMENT NAME** box, type **Preprocessing Demo**, and click **Ok** (tick).
...
6.  Point out that the **Summarize Data** module has transposed the columns from the raw data into rows, and has generated summary statistics from the 97,547 rows in the original data; verify that there are now five rows and 23 columns. Also note that there are 400 rows with missing values.
